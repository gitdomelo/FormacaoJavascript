...
export default class ImoApp {
	...
	contar() {
		return this.imóveis.length;
	}

	enumerarCidades() {
		const cidades = new Set(); // garante unicidade
		this.imóveis.forEach(imóvel => cidades.add(imóvel.cidade));
		return [...cidades]; // devolver como um array (spread operator works on iterables)
	}
