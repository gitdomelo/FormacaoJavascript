import { Predicado, Erro } from "../../../codificator.io.js";
import ImoApp from "../../../io/codificator/modelo/services/ImoApp.js";
import { promises as disco } from "fs"; // Permite aceder o "file system" com "Promises"
...
export default class { // ImoWebApp
	...
	servir(pedido, resposta) { // statusCode padrão: 200 "OK"
		resposta.setHeader("Cumprimentos-do-Formador", "email: koen@muzig.design telefone: 960025701");
	...
	servirObjetos(pedido, resposta) { // TODO: "parse" o serviço do caminho (por enquanto apenas responde a um pedido de "procurar"
		...
		pedido.on("end", async () => { // contém "await" <-- obrigatoriamente "async"
			...
			const filtro = predicado.lambda; // ...obter como lambda
			resposta.writeHead(200, { "Content-Type": this.mimes.get("json") });
			try {
				this.imoApp.procurar(filtro); // possivelmente não serão encontrados imóveis, se não, as instruções seguintes neste bloco não executarão
				const encontrados = await this.aviso; // esperar até a imoApp invoca avisar()
				resposta.end(JSON.stringify(encontrados));
			} catch (erro) {
				erro = new Erro(erro.message, predicado);
				resposta.end(JSON.stringify(erro)); // em vez de enviar resultados, enviamos o erro
			}
		});
	}
