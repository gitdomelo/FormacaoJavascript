...
export default class ImoApp {
	...
	simular() {
		...
		var imóveis = [
			...
			new Imóvel("Porto", 41.257779, -8.622000, 68000),
	...
	procurarPorCidade(cidade) {
		this.encontrados = []; // reset
		for (const imóvel of this.imóveis)
			if (imóvel.cidade === cidade)
				this.encontrados.push(imóvel);
		this.avisar(new Evento(Evento.IMÓVEIS_ENCONTRADOS, this.encontrados));
	}

	procurarPorValorMáximo(preço) {
		this.encontrados = []; // reset
		for (const imóvel of this.imóveis)
			if (imóvel.preço <= preço)
				this.encontrados.push(imóvel);
		this.avisar(new Evento(Evento.IMÓVEIS_ENCONTRADOS, this.encontrados));
	}

	procurarPorCidadeEValorMínimo(cidade, mínimo) {
		this.encontrados = []; // reset
		for (const imóvel of this.imóveis)
			if (imóvel.cidade === cidade && imóvel.preço >= mínimo)
				this.encontrados.push(imóvel);
		this.avisar(new Evento(Evento.IMÓVEIS_ENCONTRADOS, this.encontrados));
	}

	procurarPorDistância(centro, km) {
		this.encontrados = []; // reset
		for (const imóvel of this.imóveis)
