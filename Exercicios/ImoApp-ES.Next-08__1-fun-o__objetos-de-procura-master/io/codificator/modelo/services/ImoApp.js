import { Cliente, Imóvel, Coordenadas, Procura, Evento } from "../../../../codificator.io.js";
//
export default class ImoApp {
	...
	procurarPorCidadeEPreço(procura) {
		this.encontrados = []; // reset
		for (const imóvel of this.imóveis)
			if (procura.comparar(imóvel)) // intervenção do objeto "procura"
				this.encontrados.push(imóvel);
		this.avisar(new Evento(Evento.IMÓVEIS_ENCONTRADOS, this.encontrados));
	}

	procurarPorDistância(centro, km) { // este método de procura é sempre definido separadamente, pro ser demasiado diferente do anterior
		this.encontrados = []; // reset
		for (const imóvel of this.imóveis)
